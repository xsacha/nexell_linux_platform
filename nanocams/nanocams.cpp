		...
		int preview_frames)
{
	...
	for (i = 0; i < preview_frames; i++) {
		...
		for (int i = 0, j = 0; i < 800 * 600 * 2; i+= 4, j++)
		{
			p[i] = buf->virt[1][j];
			p[i+1] = buf->virt[0][j*2];
			p[i+2] = buf->virt[2][j];
			p[i+3] = buf->virt[0][j*2+1];
		}
		cv::cvtColor(mYUV, mRGB, CV_YUV2BGR_Y422, 3);
		cv::imshow("preview", mRGB);
		cv::waitKey(1);
...
static void show_usage(char *name)
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr, "Camera demo for S5P6818 board\n");
}
...
int main(int argc, char *argv[])
{
	...
	if (ret >= 0 && width_p > 0) {
		...
		do_preview(bufs, width_p, height_p, count_p);
	}
